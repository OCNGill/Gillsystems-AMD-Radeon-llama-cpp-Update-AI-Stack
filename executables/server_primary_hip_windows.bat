...
start "primary-node JSON Export" /min "%COMSPEC%" /c ""%PYTHON_EXE%" "%PROXY_SCRIPT%" --listen-host %PUBLIC_HOST% --listen-port %PUBLIC_PORT% --upstream-host %UPSTREAM_HOST% --upstream-port %UPSTREAM_PORT% --logs-dir "%LOG_DIR%" --node-prefix %NODE_PREFIX% --pid-file "%PROXY_PID_FILE%""
