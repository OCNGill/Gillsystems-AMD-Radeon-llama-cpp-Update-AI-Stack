...
set "SELF_LAUNCH_MARKER=--gillsystems-primary-child-window"
if /I not "%~1"=="--dry-run" if /I not "%~1"=="%SELF_LAUNCH_MARKER%" (
  start "primary-node Round 5" cmd.exe /k ""%~f0" %SELF_LAUNCH_MARKER% %*"
  exit /b 0
)

if /I "%~1"=="%SELF_LAUNCH_MARKER%" shift

title primary-node Round 5

set "MODEL_FILENAME=gemma-4-31B.Q4_K_M.gguf"
set "MODEL_PATH="
if defined GILLSYSTEMS_PRIMARY_MODEL_PATH (
  set "MODEL_PATH=%GILLSYSTEMS_PRIMARY_MODEL_PATH%"
) else (
  for %%P in (
...
if not exist "%MODEL_PATH%" (
  ...
  echo [Gillsystems] Set GILLSYSTEMS_PRIMARY_MODEL_PATH to override the detected model path.
  pause
  exit /b 1
)
