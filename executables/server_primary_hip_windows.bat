...
set "NODE_PREFIX=gillsystems_cluster-primary"
set "PUBLIC_HOST=192.0.2.10"
...
if defined GILLSYSTEMS_PRIMARY_MODEL_PATH set "MODEL_PATH=%GILLSYSTEMS_PRIMARY_MODEL_PATH%"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%"
if defined GILLSYSTEMS_PRIMARY_PYTHON set "PYTHON_EXE=%GILLSYSTEMS_PRIMARY_PYTHON%"
...
echo Starting primary-node LLM Server..LOCKED IN RADEON 7900XTX!
...
if /I "%~1"=="--dry-run" (
  ...
  echo.
  echo Launch Command:
  echo   llama-server.exe -m "%MODEL_PATH%" -c 49152 -n 2048 -ngl 99 -fa on -np 1 -b 2048 -ub 512 --cache-type-k q8_0 --cache-type-v q8_0 --cache-ram 0 --chat-template gemma --prio 2 --port %UPSTREAM_PORT% --host %UPSTREAM_HOST% --temperature 1.0 --top-k 64 --top-p 0.95 --min-p 0.05 --repeat-penalty 1.15 --repeat-last-n 128 --reasoning-format none --jinja --context-shift --ui-config "{\"chatFormat\":\"auto\"}" --log-file "%SERVER_LOG%" --log-timestamps --metrics --no-mmap
  exit /b 0
)
...
if not defined PYTHON_EXE (
  echo [Gillsystems] ERROR: Python 3 was not found. Install Python or set GILLSYSTEMS_PRIMARY_PYTHON.
  pause
  exit /b 1
)
...
start "primary-node JSON Export" /min "%COMSPEC%" /c ""%PYTHON_EXE%" %PYTHON_ARGS% "%PROXY_SCRIPT%" --listen-host %PUBLIC_HOST% --listen-port %PUBLIC_PORT% --upstream-host %UPSTREAM_HOST% --upstream-port %UPSTREAM_PORT% --logs-dir "%LOG_DIR%" --node-prefix %NODE_PREFIX% --pid-file "%PROXY_PID_FILE%""
