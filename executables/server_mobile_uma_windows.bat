...
echo Starting mobile-node LLM Server... Tier 2 Edge Profile.

set "MODEL_FILENAME=gemma-4-E4B.Q6_K.gguf"
set "MODEL_PATH="
if defined GILLSYSTEMS_MOBILE_MODEL_PATH (
  set "MODEL_PATH=%GILLSYSTEMS_MOBILE_MODEL_PATH%"
) else (
  for %%P in (
...
if not exist "%MODEL_PATH%" (
  echo [Gillsystems] ERROR: model not found at "%MODEL_PATH%"
    echo [Gillsystems] Set GILLSYSTEMS_MOBILE_MODEL_PATH to override the detected model path.
    pause
    exit /b 1
)
