...
- **`executables/server_mobile_uma_windows_example.bat`** — Dedicated Windows Tier 2 server-only launcher for the Laptop/Vega 6 node with path fallbacks for mirrored source-root, active build tree, and canonical install roots
- **`executables/server_deck_vulkan_linux_example.sh`** — Dedicated Linux Tier 2 server-only launcher for the Steam Deck node with path fallbacks for mirrored source-root, active build tree, and canonical install roots
