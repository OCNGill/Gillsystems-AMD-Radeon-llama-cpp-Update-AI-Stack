...
- **Main launcher path bug fixed:** The Main Windows launcher now prefers the canonical `C:\Models\Working_Models\` root and supports `GILLSYSTEMS_PRIMARY_MODEL_PATH` instead of assuming a single broken model path.
...
* **Main Rig:** `192.0.2.10:8010` (Primary Orchestrator)
* **HTPC Node:** `192.0.2.42:8011` (High-Precision Processing Edge)
* **Laptop Node:** `192.0.2.93:8012` (Telemetry / Sync Worker)
* **Steam Deck:** `192.0.2.139:8013` (Local Validation Node)
