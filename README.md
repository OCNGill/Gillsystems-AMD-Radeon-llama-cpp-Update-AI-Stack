...
**Main notes:** The main launcher keeps the Dense 31B model as the highest-quality node, prefers the canonical `C:\Models\Working_Models\` root, accepts `GILLSYSTEMS_PRIMARY_MODEL_PATH` as an override, and resolves portable rocBLAS support files when present. As of Round 6, uses `--chat-template gemma` (GGUF-embedded template) — the `--chat-template-file` Jinja file that caused crash-on-startup has been removed.
