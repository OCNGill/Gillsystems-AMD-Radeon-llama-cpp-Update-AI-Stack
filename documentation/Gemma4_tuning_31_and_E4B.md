...
|------|----|----|-----|------|-----------|-----|-------------|---------|-------|
| **primary-node** | `192.0.2.10` | Windows 11 Pro | Radeon RX 7900 XTX | 24 GB GDDR6 | 48 GB DDR5 | Ryzen 9 5900X (12C/24T) | `gfx1100` (Navi 31 / RDNA 3) | ROCm/HIP SDK | `gemma-4-31B.Q4_K_M.gguf` |
| **desktop-node** | `192.0.2.42` | Kubuntu | Radeon RX 7600 | 8 GB GDDR6 | 16 GB DDR4 | Ryzen 5 5600G (6C/12T) | `gfx1102` (Navi 33 / RDNA 3) | ROCm/HIP | `gemma-4-E4B.Q6_K.gguf` |
| **mobile-node** | `192.0.2.93` | Windows 10 | Radeon Vega 6 (integrated) | Shared (UMA) | 20 GB | Ryzen 5 4500U (6C/6T) | `gfx90c` (Renoir / Vega) | Vulkan (with HIP UMA) | `gemma-4-E4B.Q6_K.gguf` |
| **deck-node** | `192.0.2.139` | SteamOS (Arch) | AMD RDNA 2 APU (Van Gogh) | Shared 16 GB unified | 16 GB LPDDR5 | Zen 2 (4C/8T) | `gfx1033` (Van Gogh / RDNA 2) | Vulkan | `gemma-4-E4B.Q6_K.gguf` |

### Network Topology
```
Main Rig    192.0.2.10:8010  (Public) ← 127.0.0.1:18010  (Upstream llama-server)
HTPC        192.0.2.42:8011   (Direct llama-server)
Laptop      192.0.2.93:8012   (Direct llama-server)
Steam Deck  192.0.2.139:8013  (Direct llama-server)
```
...
|------|------------|----------|----------------|
| `executables/server_primary_hip_windows.bat` | primary-node | Windows Batch | Model: `C:\Models\` (hardcoded + `GILLSYSTEMS_PRIMARY_MODEL_PATH` override). Server: `C:\Gillsystems\llama.cpp\bin\` |
| `executables/server_desktop_rocm_linux.sh` | desktop-node | Linux Bash | Model: `~/Desktop/Models/` + ZFS pool + `GILLSYSTEMS_DESKTOP_MODEL_PATH` override. Server: `/opt/gillsystems/llama.cpp/bin/` + source-tree fallback |
| `executables/server_mobile_uma_windows.bat` | mobile-node | Windows Batch + PowerShell | Model: `~/Desktop/Models/` + 5 fallback paths + `GILLSYSTEMS_MOBILE_MODEL_PATH`. Server: multi-path discovery + canonical `C:\Gillsystems\llama.cpp\bin\` |
| `executables/server_deck_vulkan_linux.sh` | deck-node | Linux Bash | Model: `~/Desktop/Models/` + ZFS pool + `GILLSYSTEMS_DECK_MODEL_PATH`. Server: `~/src/llama.cpp/` + `/opt/gillsystems/` |
